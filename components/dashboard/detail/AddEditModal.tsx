...
export default function AddEditModal({
  ...
  useEffect(() => {
    ...
    if (isMedia) {
      ...
      setTMode(initial?.transcriptText ? "text" : initial?.transcriptVttUri ? "file" : "none");
  ...
  const icon: React.ComponentProps<typeof MaterialCommunityIcons>["name"] =
    isMedia ? (courseType === "Video" ? "movie-open" : "music-note") : "file-document-outline";
  const choose = isMedia ? `Choose ${courseType === "Video" ? "video" : "audio"} file` : "Type or paste your text…";

  const acceptMainMimes = useMemo(() => (courseType === "Video" ? ["video/*"] : ["audio/*"]), [courseType]);
  ...
  const pickTranscriptFile = async () => {
    const res = await DocumentPicker.getDocumentAsync({ type: ["text/vtt", "application/x-subrip", "text/plain"] });
    if (res.canceled || !res.assets?.length) return;
    const f = res.assets[0];
  ...
  const canSubmit =
    title.trim().length > 0 &&
    (isMedia
      ? !!fileUri && (tMode === "none" || (tMode === "file" ? !!transcriptVttUri : hasMeaningfulHtml(transcriptText)))
  ...
  const handleSubmit = () => {
    ...
    const item: LessonItem = isMedia
      ? {
          ...
          transcriptVttUri: tMode === "file" ? transcriptVttUri : undefined,
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={s.overlay}>
        <View style={s.sheet}>
          <Text style={s.sheetTitle}>{mode === "edit" ? `Edit ${courseType}` : `Add ${courseType}`}</Text>
          ...
          {isMedia ? (
            <Pressable onPress={pickMainFile} style={s.pickBtn}>
              ...
              <Text style={s.pickText}>{fileName ?? choose}</Text>
            </Pressable>
          ) : (
            <View style={{ borderRadius: 12, overflow: "hidden", borderWidth: 0.5, borderColor: "#d0d4db" }}>
          ...
          {isMedia && (
            <>
              ...
              <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
                {(["none", "file", "text"] as const).map((m) => (
                  <Pressable
                    key={m}
                    onPress={() => setTMode(m)}
                    style={[
                      { paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10, backgroundColor: "#eef1f6" },
                      tMode === m && { backgroundColor: "#dfe6f7" },
                    ]}
                  >
                    <Text style={[{ fontSize: 12, opacity: 0.9 }, tMode === m && { fontWeight: "700" }]}>
                      {m.toUpperCase()}
                    </Text>
                  </Pressable>
                ))}
              ...
              {tMode === "file" && (
                <Pressable onPress={pickTranscriptFile} style={s.pickBtn}>
                  <MaterialCommunityIcons name="subtitles-outline" size={18} color={ui.modal.iconPrimary} />
                  <Text style={s.pickText}>{transcriptVttUri ? "Transcript selected" : "Choose .srt/.vtt/.txt"}</Text>
                </Pressable>
              )}
          ...
          <View style={s.actions}>
            ...
            >
              <Text style={{ fontWeight: "700", color: !canSubmit ? ui.modal.textMuted : ui.modal.textOnPrimary }}>
                {mode === "edit" ? "Save" : "Add"}
              </Text>
            </Pressable>
          </View>
